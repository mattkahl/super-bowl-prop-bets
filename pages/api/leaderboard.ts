...
import { Answer, QuestionStatus, ScoredSubmission, Submission } from '@/src/types';
...
export default async function handler(
  ...
  const allTentativeScores = _.uniq(scoredSubmissions.map((submission) => submission.tentativeScore)).sort().reverse();
  const allFinalScores = _.uniq(scoredSubmissions.map((submission) => submission.currentScore)).sort().reverse();

  for (const submission of scoredSubmissions) {
    submission.tentativePlace = String(allTentativeScores.indexOf(submission.tentativeScore) + 1);
    submission.currentPlace = String(allFinalScores.indexOf(submission.currentScore) + 1);
